...
def telethon_session():
    ...
    try:
        with TelegramClient(StringSession(), API_ID, API_HASH) as jmthon:
            print("Generating a string session for •᥉ρᎥժᥱᖇ•")
            try:
                jmthon.send_message(
                    "me",
                    f"𓆘هذا هو كود سيشن تيليثون الخاص بك:\n\n`{jmthon.session.save()}`\n\nلا تشارك هذا الكود مع اي مستخدم !𓆘\n@EE_20",
                )
                print(
        ...
        print(er)
        print("If you think It as a Bug, Report to @EE_20\n\n")
...
def pyro_session():
    ...
    with Client(":memory:", api_id=API_ID, api_hash=API_HASH) as pyro:
        ...
        pyro.send_message(
            "me",
            f"`{ss}`\n\n𓆘الكود الذي في الاعلى هو كود سيشن بايروجرام لا تشاركه مع اي احد كود مهم وخطير𓆘",
        )
