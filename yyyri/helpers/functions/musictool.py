...
async def song_download(url, event, quality="128k", video=False, title=True):
    ...
    with contextlib.suppress(Exception):
        ...
        if stderr:
            return await edit_or_reply(event, f"𓆘𓆘خطأ ::𓆘𓆘 `{stderr}`")
    ...
    if not os.path.exists(media_file):
        ...
        return await edit_or_reply(event, f"𓆘𓆘- عذرا لم استطع ايجاد الطلب الخاص بك--")
    await edit_or_reply(event, f"𓆘𓆘- جار الرفع انتظر قليلا𓆘𓆘")
