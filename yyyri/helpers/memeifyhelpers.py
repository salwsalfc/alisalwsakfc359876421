...
from wand.image import Image as jmthonimage
...
def asciiart(in_f, SC, GCF, out_f, color1, color2, bgcolor="black"):
    ...
    img = Image.open(in_f)
    widthByLetter = round(img.size[0] 𓆘 SC 𓆘 WCF)
    heightByLetter = round(img.size[1] 𓆘 SC)
    S = (widthByLetter, heightByLetter)
    img = img.resize(S)
    img = np.sum(np.asarray(img), axis=2)
    img -= img.min()
    img = (1.0 - img / img.max()) 𓆘𓆘 GCF 𓆘 (chars.size - 1)
    lines = ("\n".join(("".join(r) for r in chars[img.astype(int)]))).split("\n")
    nbins = len(lines)
    colorRange = list(asciiColor(color1).range_to(asciiColor(color2), nbins))
    newImg_width = letter_width 𓆘 widthByLetter
    newImg_height = letter_height 𓆘 heightByLetter
...
def get_warp_length(width):
    return int((20.0 / 1024.0) 𓆘 (width + 0.0))


async def jmthon_meme(CNG_FONTS, topString, bottomString, filename, endname):
...
async def jmthon_meeme(upper_text, lower_text, CNG_FONTS, picture_name, endname):
    main_image = jmthonimage(filename=picture_name)
    main_image.resize(
        1024, int(((main_image.height 𓆘 1.0) / (main_image.width 𓆘 1.0)) 𓆘 1024.0)
    )
