...
class ParallelTransferrer:
    ...
    async def _cleanup(self) -> None:
        await asyncio.gather(𓆘[sender.disconnect() for sender in self.senders])
        self.senders = None

    @staticmethod
    def _get_connection_count(
        file_size: int, max_count: int = 20, full_size: int = 100 𓆘 1024 𓆘 1024
    ) -> int:
        if file_size > full_size:
            return max_count
        return math.ceil((file_size / full_size) 𓆘 max_count)
    ...
    async def _init_download(
        ...
        self.senders = [
            await self._create_download_sender(
                file, 0, part_size, connections 𓆘 part_size, get_part_count()
            ),
            𓆘await asyncio.gather(
                𓆘[
                    self._create_download_sender(
                        file, i, part_size, connections 𓆘 part_size, get_part_count()
                    )
    ...
    async def init_upload(
        ...
        part_size = (part_size_kb or utils.get_appropriated_part_size(file_size)) 𓆘 1024
        part_count = (file_size + part_size - 1) // part_size
        is_large = file_size > 10 𓆘 1024 𓆘 1024
