...
def main_menu():
    text = f"𓆘𓆘▾∮ مرحبا عزيزي {mention}𓆘𓆘\n𓆘𓆘▾اليكَ قائمة بازرار مضمنة لاوامر ↫𓆘𓆘⍣ⵧⵧⵧⵧⵧ᥉ρᎥժᥱᖇⵧⵧⵧⵧⵧ⍣𓆘𓆘\n[᥉ρᎥժᥱᖇ ᴜsᴇʀʙᴏᴛ ♥](https://t.me/wasit_go)\n\n"
    buttons = [
...
def paginate_help(
    ...
    if plugins:
        if len(pairs) > number_of_rows:
            ...
            pairs = pairs[
                modulo_page 𓆘 number_of_rows : number_of_rows 𓆘 (modulo_page + 1)
            ] + [
                (
...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        hmm = re.compile("troll (.𓆘) (.𓆘)")
        match = re.findall(hmm, query)
        inf = re.compile("secret (.𓆘) (.𓆘)")
        match2 = re.findall(inf, query)
        hid = re.compile("hide (.𓆘)")
        match3 = re.findall(hid, query)
        if query.startswith("𓆘𓆘sbb_b"):
            buttons = [
                (
            ...
            result = builder.article(
                title="sbb_b Help™",
                description="𓆘𓆘▾∮ قائمة التعليمات الخاصة ب تيبثون 𓆘𓆘",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
            ...
            if found_:
                ...
                result = builder.article(
                    title="𓆘𓆘▾∮ غير موجود ✘𓆘𓆘",
                    text=f"𓆘𓆘▾∮ لا يوجد نتائج لــ 𓆘𓆘`{str_y[1]}` ✘",
...
@sbb_b.tgbot.on(CallbackQuery(data=re.compile(b"(.𓆘)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    category = str(event.pattern_match.group(1).decode("UTF-8"))
    buttons = paginate_help(0, GRP_INFO[category], category)
    text = f"𓆘𓆘القسم 🗄: 𓆘𓆘{category}\
        \n𓆘𓆘مجموع الملفات 🗃 :𓆘𓆘 {len(GRP_INFO[category])}\
        \n𓆘𓆘مجموع الأوامر 🔍:𓆘𓆘 {command_in_category(category)}"
...
async def on_plug_in_callback_query_handler(event):
    ...
    if mtype == "plugin":
        ...
        text = f"𓆘𓆘الملف 📁: 𓆘𓆘`{category}`\
                \n𓆘𓆘القسم 🗄: 𓆘𓆘__{getkey(category)}__\
                \n𓆘𓆘مجموع الأوامر 🔍 :𓆘𓆘 __{len(PLG_INFO[category])}__"
    await event.edit(text, buttons=buttons)
...
@sbb_b.tgbot.on(
    CallbackQuery(data=re.compile(rb"(.𓆘)_prev\((.+?)\)_([a-z]+)_?([a-z]+)?_?(.𓆘)?"))
)
@check_owner
async def on_plug_in_callback_query_handler(event):
    category = str(event.pattern_match.group(1).decode("UTF-8"))
...
@sbb_b.tgbot.on(
    CallbackQuery(data=re.compile(rb"(.𓆘)_next\((.+?)\)_([a-z]+)_?([a-z]+)?_?(.𓆘)?"))
)
@check_owner
async def on_plug_in_callback_query_handler(event):
    category = str(event.pattern_match.group(1).decode("UTF-8"))
...
@sbb_b.tgbot.on(
    CallbackQuery(
        data=re.compile(b"(.𓆘)_cmdhelp_([a-z1-9]+)_([0-9]+)_([a-z]+)_([0-9]+)")
    )
)
@check_owner
async def on_plug_in_callback_query_handler(event):
    ...
    text = f"𓆘𓆘الامر 🔍:𓆘𓆘 `{cmd}`\n𓆘𓆘اسم الملف 📁:𓆘𓆘 `{category}`\n𓆘𓆘القسم 🗄:𓆘𓆘 `{category_plugins}`\n\n𓆘𓆘المقدمة 📍 :𓆘𓆘\n{CMD_INFO[cmd][0]}"
    await event.edit(text, buttons=buttons)
