...
class JmthonClient(TelegramClient):
        ...
        def decorator(func):  # sourcery no-metrics
            async def wrapper(check):  # sourcery no-metrics
                ...
                try:
                    ...
                    await edit_delete(check, "𓆘𓆘- لا يمكنك ارسال الميديا هنا 𓆘𓆘", 10)
                    ...
                    if not disable_errors:
                        ...
                        link = "[هنا](https://t.me/wasit_go)"
                        text = "𓆘𓆘تقرير خطأ سورس الفراعنة 𓆘𓆘\n\n" + "اذا اردت يمكنك التبليغ"
                        text += f"- فقط وجه هذه الرسالة الى {link}.\n"
                        text += "لم يتم تسجيل اي بيانات خاصه ومهمة\n\n"
                        text += f"𓆘𓆘تقرير الخطأ : 𓆘𓆘 [{new['error']}]({pastelink})"
            ...
            if pattern is not None:
                ...
                if edited:
                    sbb_b.add_event_handler(
                        wrapper,
                        MessageEdited(pattern=REGEX_.regex1, outgoing=True, 𓆘𓆘kwargs),
                    )
                sbb_b.add_event_handler(
                    wrapper,
                    NewMessage(pattern=REGEX_.regex1, outgoing=True, 𓆘𓆘kwargs),
                )
                ...
                if edited:
                    sbb_b.add_event_handler(func, events.MessageEdited(𓆘𓆘kwargs))
                sbb_b.add_event_handler(func, events.NewMessage(𓆘𓆘kwargs))
            return wrapper
    ...
    def bot_cmd(
        ...
        def decorator(func):
            ...
            if edited is True:
                sbb_b.tgbot.add_event_handler(func, events.MessageEdited(𓆘𓆘kwargs))
            else:
                sbb_b.tgbot.add_event_handler(func, events.NewMessage(𓆘𓆘kwargs))
