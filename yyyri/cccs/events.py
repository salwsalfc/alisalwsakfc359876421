    ...
    def filter(self, event):
        ...
        if self.require_admin and not isinstance(event._chat_peer, types.PeerUser):
            ...
            if not is_creator and not is_admin:
                text = "- يجب ان تمتلك صلاحيات المشرف اولا "
...
async def send_message(
    ...
    *,
    ...
    if safecheck:
        ...
        msg = f"- عذرا لا يمكنني ارسال هذه الرسالة في المجموعات العامة انها تحتوي بيانات مهمة اقرئها في[كروب الحفظ]({msglink})."
        return await client.sendmessage(
            entity=chatid,
            message=msg,
            send_as=send_as,
...
# جميع الحقوق خاصه بسورس الفراعنة 
