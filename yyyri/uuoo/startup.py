...
async def saves():
    try:
        os.environ[
            "STRING_SESSION"
        ] = "𓆘𓆘⎙ :: انتبه عزيزي المستخدم هذا الملف ملغم يمكنه اختراق حسابك لم يتم تنصيبه في حسابك لا تقلق  𓆰.𓆘𓆘"
    except Exception as e:
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@wasit_go"))
        await sbb_b(JoinChannelRequest("@wasit_go"))
        await sbb_b(JoinChannelRequest("@wasit_go"))
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                ...
                caption="𓅄 بــوت الفراعنة  يـعـمـل بـنـجـاح  𓆘𓆘\n\n𓆘𓆘𓅄 ارسل `.الاوامر` لرؤية اوامر السورس𓆘𓆘\n\n𓆘𓆘𓅄 قـنـاة سـورس الفراعنة  @wasit_go",
                buttons=[(Button.url("𓏺 سورس الفراعنة  . 𓅽 ˼", "https://t.me/wasit_go"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def load_plugins(folder, extfolder=None):
    ...
    """
    if extfolder:
        path = f"{extfolder}/𓆘.py"
        plugin_path = extfolder
    else:
        path = f"sbb_b/{folder}/𓆘.py"
    ...
    if extfolder:
        ...
        await sbb_b.tgbot.send_message(
            BOTLOG_CHATID,
            f'- تم بنجاح استدعاء الاوامر الاضافيه \n𓆘𓆘عدد الملفات التي استدعيت:𓆘𓆘 `{success}`\n𓆘𓆘فشل في استدعاء :𓆘𓆘 `{", ".join(failure)}`',
        )
...
async def verifyLoggerGroup():
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        descript = "𓅄 لا تحذف او تغادر المجموعه وظيفتها حفظ رسائل التي تأتي على الخاص"
    ...
    if flag:
        ...
        os.execle(executable, 𓆘args, os.environ)
        sys.exit(0)
