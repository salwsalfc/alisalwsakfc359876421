...
@sbb_b.tgbot.on(CallbackQuery(data=re.compile(b"secret_(.𓆘)")))
async def on_plug_in_callback_query_handler(event):
    timestamp = int(event.pattern_match.group(1).decode("UTF-8"))
    if os.path.exists("./sbb_b/secret.txt"):
        jsondata = json.load(open("./sbb_b/secret.txt"))
        try:
            message = jsondata[f"{timestamp}"]
            userid = message["userid"]
            ids = userid + [sbb_b.uid]
