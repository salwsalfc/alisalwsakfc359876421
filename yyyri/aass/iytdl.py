...
@sbb_b.ar_cmd(pattern="يوت(?:\s|$)([\s\S]𓆘)")
async def iytdl_inline(event):
    ...
    if not input_url:
        return await edit_delete(
            event, "𓆘𓆘يجب عليك الرد على الرابط او كتابة عنوان مع الامر𓆘𓆘"
        )
    jmrhonevent = await edit_or_reply(
        event, f"🔎 جار البحث في اليوتيوب: `'{input_url}'`"
    )
    flag = True
    cout = 0
    results = None
    while flag:
        ...
        if cout > 5:
            flag = False
    if results:
        await jmrhonevent.delete()
        await results[0].click(event.chat_id, reply_to=reply_to_id, hide_via=True)
    else:
        await jmrhonevent.edit("عذرا لم يتم اجاد اي نتائج")


@sbb_b.tgbot.on(
    CallbackQuery(
        data=re.compile(b"^ytdl_download_(.𓆘)_([\d]+|mkv|mp4|mp3)(?:_(a|v))?")
    )
)
@check_owner
async def ytdl_download_callback(c_q: CallbackQuery):  # sourcery no-metrics
    ...
    if str(choice_id).isdigit():
        ...
        if choice_id == 0:
            await c_q.answer("🔄  جار تنفيذ العملية ...", alert=False)
    ...
    upload_msg = await c_q.client.send_message(
        BOTLOG_CHATID, "- جار الرفع على التليجرام..."
    )
    ...
    for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "𓆘")):
        if _path.lower().endswith((".jpg", ".png", ".webp")):
            ...
            _fpath = _path
    if not _fpath:
        await edit_delete(upload_msg, "لم يتم ايجاد اي شي")
        return
    if not thumb_pic:
...
@sbb_b.tgbot.on(
    CallbackQuery(data=re.compile(b"^ytdl_(listall|back|next|detail)_([a-z0-9]+)_(.𓆘)"))
)
@check_owner
