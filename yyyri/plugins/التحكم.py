...
async def hi(event):
    await edit_or_reply(
        event,
        "𓆘𓆘[ . 𝐒𝐏𝐈𝐃𝐄𝐑 - 𝐂𝐎𝐌𝐌𝐀𝐍𝐃𝐒 ♛ .](t.me/Tepthone)𓆘𓆘\n⎈━━━⊶𐂡•المطور علي •𐂡⊷━━━⎈\n\n• اوامر التحكم تتيح لمستخدم اخر ان يستخدم اوامر تنصيبك وهو غير منصب الفراعنة حيث سيصبح متحكم في اوامر تنصيبك يرجى الانتباه من انه سيحصل على صلاحيات الاوامر  وقد يسبب خطورة لك اذا كنت لا تثق في المستخدم الذي اضفته. \n\n\n`.التحكم` تفعيل/تعطيل\n• تستخدم هذه الميزة لتفعيل/لتعطيل خاصية التحكم بأوامر تنصيبك للمستخدمين الذين أضفتهم في قائمة المتحكمين\n\n`.المتحكمين`\n• لعرض المستخدمين الذين اضفتهم الى قائمة المستخدمين المتحكمين\n\n`.اضف متحكم`\n• بالرد على المستخدم لأضافته متحكم في تنصيب الفراعنة الخاص بك\n\n`.ازالة متحكم`\n• بالرد على المستخدم لازالته من قائمة المتحكمين",
        link_preview=False,
    )
...
async def chat_blacklist(event):
    ...
    if input_str == "تفعيل":
        if gvarstatus("sudoenable") is not None:
            return await edit_delete(event, "𓆘𓆘- ميزة التحكم مُفعلة بالأصل𓆘𓆘")
        addgvar("sudoenable", "true")
        text = "𓆘𓆘- تم بنجاح تفعيل ميزة التحكم𓆘𓆘\n"
        if len(sudousers) != 0:
            text += (
                "𓆘𓆘جار الان اعادة التشغيل لتطبيق التغيير يرجى الأنتظار بعض الدقائق𓆘𓆘"
            )
        ...
        text += "𓆘𓆘انت لم تقم باضافة اي شخص الى قائمة المتحكمين بالأصل ارسل `.اوامر التحكم`𓆘𓆘"
    ...
    if gvarstatus("sudoenable") is not None:
        delgvar("sudoenable")
        text = "𓆘𓆘تم بنجاح تعطيل ميزة التحكم𓆘𓆘\n"
        if len(sudousers) != 0:
            text += (
                "𓆘𓆘جار الان اعادة التشغيل لتطبيق التغيير يرجى الأنتظار بعض الدقائق𓆘𓆘"
            )
    ...
    await edit_delete(event, "𓆘𓆘- ميزة التحكم بالأصل مُعطلة𓆘𓆘")


@sbb_b.ar_cmd(pattern="اضف متحكم(?:\s|$)([\s\S]𓆘)")
async def add_sudo_user(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if replied_user is None:
        return
    if replied_user.id == event.client.uid:
        return await edit_delete(event, "𓆘𓆘- لا يمكنك أضافة نفسك كمستخدم متحكم𓆘𓆘")
    ...
    output += "𓆘𓆘جار الان اعادة التشغيل لتطبيق التغيير يرجى الأنتظار بعض الدقائق𓆘𓆘"
    msg = await edit_or_reply(event, output)
    await event.client.reload(msg)


@sbb_b.ar_cmd(pattern="ازالة متحكم(?:\s|$)([\s\S]𓆘)")
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if replied_user is None:
        return
...
async def _(event):
    ...
    if len(sudochats) == 0:
        return await edit_delete(
            event, "𓆘𓆘• يبدو انك لم تقوم بأضافة مستخدم متحكم الفراعنة الخاص بك𓆘𓆘"
        )
    result = "𓆘𓆘قائمة المستخدمين المتحكمين بتنصيب الفراعنة :𓆘𓆘\n\n"
    for chat in sudochats:
        result += f"𓆘𓆘• الأسم:𓆘𓆘 {mentionuser(sudousers[str(chat)]['chat_name'],sudousers[str(chat)]['chat_id'])}\n"
        result += f"𓆘𓆘• ايدي الدردشة :𓆘𓆘 `{chat}`\n"
        username = f"@{sudousers[str(chat)]['chat_username']}" or "غير معرف"
        result += f"𓆘𓆘• المعرف :𓆘𓆘 {username}\n"
