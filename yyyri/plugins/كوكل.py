...
async def scam(results, lim):
    ...
    imglinks = []
    counter = 0
    pattern = r"^,\[\"(.𓆘[.png|.jpg|.jpeg])\",[0-9]+,[0-9]+\]$"
...
async def gsearch(q_event):
    ...
    catevent = await edit_or_reply(q_event, "𓆘𓆘▿∲ جـاري البحـث انتـظر𓆘𓆘")
    ...
    ysearch = YahooSearch()
    try:
        gresults = await gsearch.async_search(𓆘search_args)
    except NoResultsOrTrafficError:
        try:
            gresults = await bsearch.async_search(𓆘search_args)
        except NoResultsOrTrafficError:
            try:
                gresults = await ysearch.async_search(𓆘search_args)
            except Exception as e:
                return await edit_delete(catevent, f"𓆘𓆘᯽︙ خطـأ  :𓆘𓆘\n`{str(e)}`", time=10)
    msg = ""
    for i in range(lim):
    ...
    await edit_or_reply(
        ...
        link_preview=False,
        aslink=True,
        linktext=f"𓆘𓆘⎊ نتائـج البحـث 𓆘𓆘 `{match}` :",
    )
    if BOTLOG:
        await q_event.client.send_message(
            BOTLOG_CHATID,
            "𓆘𓆘⎊ نتـائج بحـث جـوجـل  𓆘𓆘" + match + "𓆘𓆘تم تنـفيذهـا بنجـاح 𓆘𓆘",
        )
...
async def _(event):
    ...
    start = datetime.now()
    OUTPUT_STR = "𓆘𓆘⎊ يجـب الـرد على صـورة للـبحث عـنها 𓆘𓆘"
    if event.reply_to_msg_id:
        catevent = await edit_or_reply(event, "𓆘𓆘⎊ يـتم الـبحث عـن الـصورة انتـظر𓆘𓆘 ⏱")
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await catevent.edit("𓆘𓆘⎊ تم العثـور على نتيجـة بحـث جـوجـل 𓆘𓆘")
        headers = {
        ...
        try:
            ...
            return await edit_delete(
                catevent, "𓆘𓆘⎊ غيـر قـادر على إيجـاد صـور مـشابهـة 𓆘𓆘"
            )
        end = datetime.now()
...
async def google_search(event):
    ...
    if not input_str:
        return await edit_delete(
            event, "𓆘𓆘⎊ يجـب إعطـاء معلومـات عن البحـث 𓆘𓆘"
        )
    input_str = deEmojify(input_str).strip()
    if len(input_str) > 195 or len(input_str) < 1:
        return await edit_delete(
            event,
            "𓆘𓆘⎊ لقـد تجـاوز اسطر البحـث عـن 200 حـرف أو أن حروف البحـث فـارغه ️𓆘𓆘",
        )
    query = "#12" + input_str
