...
async def honk(event):
    ...
    if not text:
        if event.is_reply:
            ...
            return await edit_delete(event, "𓆘𓆘 ⌯︙ماذا يفترض أن يقول هونك أعطه نص𓆘𓆘")
    text = deEmojify(text)
    await event.delete()
...
async def twt(event):
    ...
    if not text:
        if event.is_reply:
            ...
            return await edit_delete(event, "𓆘𓆘⌯︙ماذا يفترض بي ان اغرد اكتي نصا𓆘𓆘")
    text = deEmojify(text)
    await event.delete()
    await hide_inlinebot(event.client, bot_name, text, event.chat_id, reply_to_id)
...
async def doge(event):
    ...
    if not text:
        if event.is_reply:
            ...
            return await edit_delete(event, "⌯︙ماذا يفترض بالكلب ان يقول اعطه نصا𓆘𓆘")
    text = deEmojify(text)
    await event.delete()
    await hide_inlinebot(event.client, bot_name, text, event.chat_id, reply_to_id)
    ...
    if not text:
        if event.is_reply:
            ...
            return await edit_delete(
                event, " ⌯︙ماذا يفترض بـ گلاكـس ان يقول اعطه نصا𓆘𓆘"
            )  # WRITED BY - @co_od - @co_od
    text = deEmojify(text)
    await event.delete()
