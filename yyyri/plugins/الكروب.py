...
@sbb_b.ar_cmd(pattern="المشرفين(?: |$)(.𓆘)")
async def _(event):
    mentions = "𓆘𓆘⌔∮ مشرفين هذه المجموعة  ✪𓆘𓆘: \n"
    reply_message = await reply_id(event)
    input_str = event.pattern_match.group(1)
    to_write_chat = await event.get_input_chat()
    chat = None
    if input_str:
        mentions = f"𓆘𓆘⪼ مشرفين في → :𓆘𓆘 {input_str} 𓆘𓆘من المجموعات ⌂ :𓆘𓆘 \n"
        try:
        ...
        if not event.is_group:
            return await edit_or_reply(event, "𓆘𓆘⌔∮ هذه ليست مجموعة ✕𓆘𓆘")
    try:
...
@sbb_b.ar_cmd(pattern="البوتات(?: |$)(.𓆘)")
async def _(event):
    mentions = "𓆘𓆘⌔∮ البوتات في هذه المجموعة 🝰 : 𓆘𓆘 \n"
    input_str = event.pattern_match.group(1)
    if not input_str:
        chat = await event.get_input_chat()
    else:
        mentions = "𓆘𓆘⌔∮ البوتات في {} من المجموعات 🝰 : 𓆘𓆘 \n".format(input_str)
        try:
            chat = await event.client.get_entity(input_str)
        except Exception as e:
...
@sbb_b.ar_cmd(pattern="الاعضاء(?: |$)(.𓆘)")
async def get_users(show):
    mentions = "𓆘𓆘مستخدمين هذه المجموعة𓆘𓆘: \n"
    await reply_id(show)
    input_str = show.pattern_match.group(1)
    if input_str:
        mentions = "𓆘𓆘⌔∮ الأعضاء في {} من المجموعات 𖤍  :𓆘𓆘 \n".format(input_str)
        try:
        ...
        if not show.is_group:
            return await edit_or_reply(show, "𓆘𓆘⌔∮ هذه ليست مجموعة ✕𓆘𓆘")
    sbb_bevent = await edit_or_reply(show, "𓆘𓆘⌔∮ جاري سحب قائمة معرّفات الأعضاء 🝛𓆘𓆘")
    try:
        if show.pattern_match.group(1):
            async for user in show.client.iter_participants(chat.id):
                if user.deleted:
                    mentions += f"\n𓆘𓆘⌔∮ الحسابات المحذوفة ⌦𓆘𓆘 `{user.id}`"
                else:
                    mentions += (
...
@sbb_b.ar_cmd(pattern="معلومات(?: |$)(.𓆘)")
async def info(event):
    sbb_bevent = await edit_or_reply(event, "𓆘𓆘⪼ يتمّ جلب معلومات الدردشة، إنتظر ⅏𓆘𓆘")
    ...
    try:
        ...
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, f"𓆘𓆘⌔∮ هناك خطأ في معلومات الدردشة ✕ : 𓆘𓆘\n`{str(e)}`"
            )
        await sbb_bevent.edit("𓆘𓆘⌔∮ حدث خطأ ما، يرجى التحقق من الأمر ⎌𓆘𓆘")
...
async def get_chatinfo(event, sbb_bevent):
    ...
    try:
        ...
        try:
            ...
            await sbb_bevent.edit("𓆘𓆘⌔∮ لم يتمّ العثور على القناة/المجموعة ✕𓆘𓆘")
            return None
        except ChannelPrivateError:
            await sbb_bevent.edit(
                "𓆘𓆘⌔∮ هذه مجموعة أو قناة خاصة أو لقد تمّ حظري منه ⛞𓆘𓆘"
            )
            return None
        except ChannelPublicGroupNaError:
            await sbb_bevent.edit("𓆘𓆘⌔∮ القناة أو المجموعة الخارقة غير موجودة ✕𓆘𓆘")
            return None
...
async def fetch_info(chat, event):  # sourcery no-metrics
    ...
    if exp_count is not None:
        chat_level = int((1 + sqrt(1 + 7 𓆘 exp_count / 14)) / 2)
