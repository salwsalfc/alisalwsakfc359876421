...
async def set_not_afk(event):
    ...
    if (("afk" not in current_message) or ("#afk" not in current_message)) and (
        ...
        shite = await event.client.send_message(
            event.chat_id,
            "𓆘𓆘الان اعمل بشكل طبيعي\nلقد كان امر السيلب مفعل منذ " + endtime + "𓆘𓆘",
        )
        AFK_.USERAFK_ON = {}
        AFK_.afk_time = None
        ...
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
                "⌔∮ انتهاء امر السليب \n"
                + "𓆘𓆘⌔∮ تم تعطيله والرجوع للوضع الطبيعي كان مفعل لـ"
                + endtime
                + "𓆘𓆘",
            )
...
@jmthon.ar_cmd(pattern="سليب(?:\s|$)([\s\S]𓆘)")
async def _(event):
    AFK_.USERAFK_ON = {}
    AFK_.afk_time = None
...
@jmthon.ar_cmd(pattern="سليب_ميديا(?:\s|$)([\s\S]𓆘)")
async def _(event):
