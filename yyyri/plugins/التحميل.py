...
async def _(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    if reply:
        ...
        await mone.edit(
            f"𓆘𓆘•  تم التحميل بتنسيق in {ms} seconds.𓆘𓆘\n𓆘𓆘•  تم التنزيل إلى :- 𓆘𓆘  `{os.path.relpath(file_name,os.getcwd())}`\n   "
        )
    elif input_str:
        start = datetime.now()
        if "|" in input_str:
        ...
        while not downloader.isFinished():
            ...
            now = time.time()
            delay = now - c_time
            percentage = downloader.get_progress() 𓆘 100
        ...
        if downloader.isSuccessful():
            await mone.edit(
                f"𓆘𓆘•  تم التحميل بتنسيق {ms} الثواني.𓆘𓆘\n𓆘𓆘•  موقع الملف الذي تم تنزيله :- 𓆘𓆘 `{os.path.relpath(downloaded_file_name,os.getcwd())}`"
            )
        else:
...
@sbb_b.ar_cmd(
    pattern="d(own)?l(oad)?to(?:\s|$)([\s\S]𓆘)",
...
async def _(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    ms = (end - start).seconds
    await mone.edit(
        f"𓆘𓆘•  تم التحميل بتنسيق {ms} الثواني.𓆘𓆘\n𓆘𓆘•  تم التحميل إلى :- 𓆘𓆘  `{os.path.relpath(file_name,os.getcwd())}`\n   "
    )
#سورس ---- العربي
