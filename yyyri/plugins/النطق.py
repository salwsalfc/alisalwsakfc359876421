...
@sbb_b.ar_cmd(pattern="انطق(?:\s|$)([\s\S]𓆘)")
async def _(event):
    ...
    if ";" in input_str:
        ...
        if not input_str:
            return await edit_or_reply(event, "𓆘𓆘- يجب استخدام الامر بشكل صحيح𓆘𓆘")
        text = input_str
        lan = "en"
    ...
    try:
        ...
        await edit_delete(
            razanevent,
            f"𓆘𓆘- عملية التحويل {text[:20]} الى مقطع صوتي في {ms} من الثواني",
        )
    except Exception as e:
        await edit_or_reply(razanevent, f"𓆘𓆘خطأ:𓆘𓆘\n`{e}`")
