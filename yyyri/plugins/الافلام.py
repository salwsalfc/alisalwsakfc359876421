...
@sbb_b.ar_cmd(pattern="فلم ([\s\S]𓆘)")
async def _(event):
    query = event.pattern_match.group(1)
    et = await edit_or_reply(event, "𓆘𓆘- جار البحث في المواقع انتظر قليلا𓆘𓆘")
    try:
        streams = get_stream_data(query)
    except Exception as e:
        return await et.edit(f"𓆘𓆘خطأ :𓆘𓆘 `{e}`")
    title = streams["title"]
    ...
    output_ = f"𓆘𓆘الفلم:𓆘𓆘\n`{title}`\n𓆘𓆘تاريخ الصدور:𓆘𓆘\n`{release_date}`"
    if imdb_score:
        output_ = f"{output_}\n𓆘𓆘IMDB: 𓆘𓆘{imdb_score}"
    if tmdb_score:
        output_ = f"{output_}\n𓆘𓆘TMDB: 𓆘𓆘{tmdb_score}"
