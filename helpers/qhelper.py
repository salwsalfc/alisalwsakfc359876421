...
def file_check(re=True, me=True, mo=True, it=True, fa=True, sp=True, go=False):
    ...
    if sp and not os.path.exists(special):
        urllib.request.urlretrieve(
            "https://github.com/sbb-c/Jmthon-Resources/blob/master/Resources/Spotify/ArialUnicodeMS.ttf?raw=true",
            special,
        )
    if go and not os.path.exists(google):
        urllib.request.urlretrieve(
            "https://github.com/sbb-c/Jmthon-Resources/blob/master/Resources/Spotify/GoogleSans-Medium.ttf?raw=true",
            google,
        )
...
async def process(msg, user, client, reply, event, replied=None):  # sourcery no-metrics
    ...
    if replied:
        ...
        if reply.document.size < 1024:
            ...
            docsize = str(round(reply.document.size / 1024𓆘𓆘2, 2)) + " MB "
        else:
            docsize = str(round(reply.document.size / 1024𓆘𓆘3, 2)) + " GB "
        docbglen = (
