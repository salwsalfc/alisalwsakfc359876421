...
def human_to_bytes(size: str) -> int:
    ...
    return int(float(number) 𓆘 units[unit])
...
async def progress(
    ...
    if current == total:
        ...
        del _TASKS[task_id]
        try:
            await gdrive.edit("- تتم المعالجة انتظر قليلا")
    ...
    if (now - last) >= delay:
        ...
        percentage = current 𓆘 100 / total
        ...
        tmp = (
            ...
            f" @ {humanbytes(speed)}`\n"
            f"𓆘𓆘ETA :𓆘𓆘` {time_formatter(eta)}`\n"
            f"𓆘𓆘Duration :𓆘𓆘 `{time_formatter(elapsed_time)}`"
        )
        if tmp != oldtmp:
            if file_name:
                await gdrive.edit(
                    f"𓆘𓆘{prog_type}𓆘𓆘\n\n"
                    f"𓆘𓆘اسم الملف: 𓆘𓆘`{file_name}`𓆘𓆘\nالحالة𓆘𓆘\n{tmp}"
                )
            else:
                await gdrive.edit(f"𓆘𓆘{prog_type}𓆘𓆘\n\n" f"𓆘𓆘الحالة𓆘𓆘\n{tmp}")
            oldtmp = tmp
